        ...
        builder:(context, snapshot) {
          if (snapshot.connectionState == ConnectionState.done){
            if (snapshot.hasData){
              ...
              orderItems.add(Align(alignment:Alignment.center, child: Text("Orders",style: const TextStyle(fontSize:20))));
              orderItems.add(SizedBox(height:10));

              final orders = snapshot.data;
              for (dynamic order in orders){
                orderItems.add(GestureDetector(onTap:(){Navigator.push(context, MaterialPageRoute(builder: (context) => OrderDetailPage(order.values.toList()[0]["order_id"])));}, child:OrderItem(order)));
              }
              ...
              Row(
                children: <Widget>[
                  ...
                  Column(
                    ...
                    children: [
                      Text("Customer: ${order.values.toList()[0]["email"]}", style:const TextStyle(fontSize:15)),
                      Text("Items: ${order.keys.toList()[0]}"),
                      Text("Ordered ${dtObject.day}/${dtObject.month}/${dtObject.year} at ${dtObject.hour}:${dtObject.minute}")
                    ],
                  ),
                ],
              ),
